    ...
    return ReactDOM.createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center px-4">
            ...
            <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-scale-in">
                {/* Header */}
                <div className="flex items-start justify-between px-6 pt-6 pb-4 border-b border-gray-100 sticky top-0 bg-white">
                    <div className="pr-4">
                ...
                <div className="px-6 py-4 space-y-5">
                    ...
                    {isPlatformOwner && ticket.tenant_name && (
                        <div className="flex items-center gap-3 p-3 bg-brand-50 border border-brand-100 rounded-xl">
                            ...
                            <div>
                                <p className="text-xs text-brand-500">Tenant</p>
                                <p className="text-sm font-semibold text-brand-800">
                                    {ticket.tenant_name} <span className="font-normal text-brand-500">@{ticket.tenant_slug}</span>
                                </p>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                        <div className="rounded-xl bg-gray-50 p-3">
                            <p className="text-xs text-gray-400 mb-1 flex items-center gap-1"><User className="w-3 h-3" />Submitted By</p>
                            <p className="font-medium text-gray-900">{ticket.created_by_name || '—'}</p>
                            <p className="text-xs text-gray-500">{ticket.created_by_email}</p>
                            <p className="text-xs text-gray-400 mt-0.5">{ticket.created_by_role}</p>
                        </div>
                        <div className="rounded-xl bg-gray-50 p-3">
                    ...
                    {ticket.context_info && Object.keys(ticket.context_info).length > 0 && (
                        <div>
                            <p className="text-xs font-medium text-gray-400 mb-2">SYSTEM CONTEXT</p>
                            <div className="bg-gray-50 rounded-xl p-3 space-y-1">
                                {Object.entries(ticket.context_info).map(([k, v]) => (
                                    <div key={k} className="flex items-start gap-2 text-xs">
                                        <span className="text-gray-400 w-24 shrink-0">{k.replace(/_/g, ' ')}</span>
                        ...
                        <div className="flex gap-3 pt-2">
                            {ticket.status === 'open' && (
                                <button
                                    ...
                                    className="flex-1 py-2.5 rounded-xl bg-amber-50 border border-amber-200 text-amber-700 text-sm font-medium hover:bg-amber-100 transition-colors disabled:opacity-50"
                                >
                            ...
                            <button
                                ...
                                className="flex-1 py-2.5 rounded-xl bg-green-50 border border-green-200 text-green-700 text-sm font-medium hover:bg-green-100 transition-colors disabled:opacity-50"
                            >
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center px-4">
            ...
            <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-scale-in">
                <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-gray-100">
                    <h2 className="text-base font-semibold text-gray-900">Open Support Ticket</h2>
                ...
                <div className="px-6 py-5 space-y-4">
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <select
                                    ...
                                    className="w-full appearance-none px-3 py-2.5 border border-gray-200 rounded-xl bg-white text-sm text-gray-900 focus:ring-2 focus:ring-brand-500 focus:border-transparent outline-none pr-8"
                                >
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            rows={5}
                            className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm text-gray-900 placeholder:text-gray-400 focus:ring-2 focus:ring-brand-500 focus:border-transparent outline-none resize-none"
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-3 gap-4">
                {[
                    { label: 'Open', count: openCount, color: 'bg-red-100', icon: <AlertCircle className="w-5 h-5 text-red-600" />, filter: 'open' },
                    { label: 'In Progress', count: inProgressCount, color: 'bg-amber-100', icon: <Clock className="w-5 h-5 text-amber-600" />, filter: 'in_progress' },
                    { label: 'Resolved', count: resolvedCount, color: 'bg-green-100', icon: <CheckCircle2 className="w-5 h-5 text-green-600" />, filter: 'resolved' },
                ].map(stat => (
                    <button
                        key={stat.filter}
                        onClick={() => setStatusFilter(statusFilter === stat.filter ? 'all' : stat.filter)}
                        className={`text-left transition-all ${statusFilter === stat.filter ? 'ring-2 ring-brand-500 rounded-xl' : ''}`}
                    >
                        <Card>
            ...
            <div className="flex items-center gap-3 flex-wrap">
                <div className="relative">
                    <select
                        ...
                        className="appearance-none pl-3 pr-8 py-2 border border-gray-200 rounded-lg bg-white text-sm text-gray-700 focus:ring-2 focus:ring-brand-500 outline-none"
                    >
                ...
                <button
                    ...
                    disabled={loading}
                    className="flex items-center gap-1.5 px-3 py-2 border border-gray-200 rounded-lg bg-white text-sm text-gray-700 hover:bg-gray-50 transition-colors disabled:opacity-50"
                >
            ...
            <Card>
                {error ? (
                    <div className="flex flex-col items-center py-12 text-center">
                        ...
                        <p className="text-gray-700 font-medium mb-1">Failed to load tickets</p>
                    ...
                    <div className="divide-y divide-gray-100">
                        {filtered.map(ticket => (
                            <button
                                ...
                                className="w-full text-left px-2 py-4 hover:bg-gray-50 transition-colors rounded-xl group"
                            >
                                <div className="flex items-start gap-4">
                                    ...
                                    <ArrowUpRight className="w-4 h-4 text-gray-300 group-hover:text-brand-500 transition-colors flex-shrink-0 mt-1" />
                                </div>
                            </button>
                        ))}
                    </div>
